...
def p_engines(d):
    ...
    return f'''<div class="page"><h2><span class="num">02</span>Где вас находят нейросети</h2>
      ...
      <div class="two">
        <div class="box cream"><h4>Сильные каналы</h4><p>Perplexity и ChatGPT уже используют данные о вас. Задача: удержать позиции и держать отзывы и факты на сайте свежими.</p></div>
        <div class="box"><h4>Слабые каналы</h4><p>Яндекс Нейро и GigaChat почти не знают бренд. Рост дадут Яндекс.Карты, отзывы и присутствие в российских каталогах.</p></div>
      </div>
...
def p_matrix(d):
    ...
    return f'''<div class="page"><h2><span class="num">03</span>В каких ответах бренд появляется, а в каких нет</h2>
      ...
      <div class="two" style="margin-top:5mm">
        ...
        <div class="box"><h4>Вывод</h4><p>Стабильное появление пока только по части запросов о кухнях и встроенной мебели. По премиальным и нишевым запросам бренд не закреплён.</p></div></div>
...
def p_groups(d):
    ...
    return f'''<div class="page"><h2><span class="num">04</span>Видимость по группам запросов</h2>
      ...
      <div class="card">{bars}</div>
      <div class="two" style="margin-top:4mm">
        <div class="box cream"><h4>Главные потери</h4><p>Премиальная и дизайнерская мебель, изделия из массива. Здесь видимость близка к нулю, а это самый маржинальный сегмент.</p></div>
        <div class="box"><h4>На что опереться</h4><p>Кухни и встроенная мебель в Москве уже дают узнаваемость. Их стоит усиливать первыми, чтобы быстрее поднять общий процент.</p></div></div>
      <div class="box"><h4>Приоритет по сегментам</h4><p>Сначала усилить кухни и встроенную мебель: это быстрый рост из текущей базы. Затем закрыть премиум и массив, где сейчас ноль и где самый дорогой клиент.</p></div>
      {footer(d)}</div>'''
...
def p_examples(d):
    ...
    return f'''<div class="page"><h2><span class="num">05</span>Примеры реальных ответов нейросетей</h2>
      ...
      {cards}
      <div class="box cream"><h4>Что показывают примеры</h4><p>Там, где у вас есть страница и отзывы по теме, нейросети называют бренд стабильно. Там, где контента и упоминаний нет (премиум, массив), вас заменяют конкурентами.</p></div>
