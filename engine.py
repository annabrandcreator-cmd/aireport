...
def build_data(brand, brand_short, site, niche, city, queries, competitors, site_info=None):
    ...
    data = {
        ...
        "recommendations": _recommendations(queries, groups, total, site_info, brand_short),
...
def _recommendations(queries, groups, total, site_info=None, brand_short="бренд"):
    """Карточки рекомендаций на языке владельца: что означает, что сделать, пример, кому передать, тип, приоритет."""
    ...
    # 4. Техническая: чек-лист для специалиста (с расшифровкой и плашкой «кому передать»)
    if blocks_ai:
        tech_plain = ("Сейчас в robots.txt часть AI-роботов закрыта (" + ", ".join(s["robots_blocks_ai"]) + "). "
                      "Пока доступ закрыт, нейросети не могут использовать сайт как источник. Это первое, что нужно поправить. "
    ...
    recs.append({
        "kind": "tech",
        "title": "Передать администратору сайта технический чек-лист",
        "plain": tech_plain,
