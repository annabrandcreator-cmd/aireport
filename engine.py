...
def generate_queries_llm(niche, city, site_info=None):
    ...
    prompt = (
        ...
        "СПИСОК КОМПАНИЙ — САМОЕ ВАЖНОЕ: минимум 7 из 10 запросов сформулируй так, чтобы ассистент в ответ ВЫДАЛ СПИСОК конкретных компаний, "
        "сайтов или магазинов: «Какие компании…», «Кто делает/продаёт…», «Где заказать/купить…», «Найди компании/сайты, которые…», "
        "«Посоветуй компании/магазины…», «В каких компаниях…». По таким запросам видно, кого нейросеть называет, а кого нет — это и есть показательный замер. "
...
_QSET_VER = "6"   # бамп при изменении промпта запросов -> старый кэш игнорируется и набор пересобирается
