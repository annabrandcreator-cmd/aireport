...
def generate_queries_tpl(niche, city):
    ...
    items = [
        (f"Где заказать {n}{inc}?",                            "Поиск и выбор"),
        (f"Какие компании делают {n}{inc}?",                   "Поиск и выбор"),
        (f"Кто делает {n}{inc}?",                              "Поиск и выбор"),
...
def _plan30(site_info=None):
    """План на месяц с ответственными за каждый блок. Структура: неделя -> группы по ролям."""
    s = site_info or {}
    svc = bool(s.get("service_pages"))
    w2_admin = ("Разместить тексты, настроить внутренние ссылки, добавить разметку Service и FAQPage" if svc
                else "Опубликовать новые страницы услуг, настроить внутренние ссылки, добавить разметку Service и FAQPage")
    return [
        {"week": "Неделя 1 · приоритетные страницы", "groups": [
            ...
            {"role": "Администратор сайта", "items": [
                "Проверить доступность страниц для индексации",
                "Проверить robots.txt, sitemap и noindex",
                "Проверить существующую техническую разметку"]},
        ]},
        {"week": "Неделя 2 · услуги", "groups": [
            ...
            {"role": "Администратор сайта", "items": [w2_admin]},
        ]},
...
def _reco_examples(niche, brand_short):
    """Конкретные примеры под нишу клиента (заголовок, кейс, отзыв), чтобы рекомендации не выглядели шаблонно. {} = откат на общие."""
    if os.environ.get("TEST_MODE") == "1":
        return {}
    ask = _first_keyed_adapter()
    if not ask or not niche:
        return {}
    prompt = (f"Ниша компании: «{niche}». Дай 3 КОНКРЕТНЫХ примера именно для этой ниши, без воды и без markdown.\n"
              "1) Заголовок страницы услуги: слабый → сильный (с конкретной услугой, аудиторией и форматом этой ниши).\n"
              "2) Структура кейса одной строкой с реальными для ниши полями: задача, что сделали, срок, результат.\n"
              "3) Полезный отзыв клиента (1–2 предложения) с конкретикой ниши, не «всё понравилось».\n"
              "Ответь СТРОГО тремя строками, начиная каждую с метки:\n"
              "HEAD: было «...» → стало «...»\n"
              "CASE: ...\n"
              "REVIEW: ...")
    try:
    ...
    for ln in (raw or "").splitlines():
        s = ln.strip()
        for key, tag in (("head", "HEAD:"), ("case", "CASE:"), ("review", "REVIEW:")):
            if s.upper().startswith(tag):
                v = s[len(tag):].strip().strip("«»\"").strip()
                if len(v) >= 8:
                    out[key] = v
    return out
...
def _recommendations(queries, groups, total, site_info=None, brand_short="бренд", niche=""):
    ...
    recs.append({
        ...
        "example": (f"Пример для вашей ниши: {rex['head']}. В заголовке видно услугу, аудиторию и формат, текст — естественный, с фактами."
                    if rex.get('head') else
    ...
    recs.append({
        ...
        "example": (f"Пример структуры кейса для вашей ниши: {rex['case']}. Заголовок — говорящий, с типом клиента и сутью проекта."
                    if rex.get('case') else
    ...
    recs.append({
        ...
        "example": (f"Полезный отзыв вместо «Всё понравилось, рекомендуем»: «{rex['review']}». Чем конкретнее факты, тем охотнее нейросеть их процитирует."
