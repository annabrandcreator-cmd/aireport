...
_DEPERS_VERB = (r"(?:найди(?:те)?|посоветуй(?:те)?|подскажи(?:те)?|порекомендуй(?:те)?|подбери(?:те)?|"
                r"предложи(?:те)?|перечисли(?:те)?|назови(?:те)?|покажи(?:те)?|помоги(?:те)?(?:\s+найти)?|"
                r"дай(?:те)?(?:\s+список)?)")
...
def generate_queries_llm(niche, city, site_info=None):
    ...
    if product_biz:
        ...
        role_block = ("\nЭТО ЗАВЕДЕНИЕ ИЛИ МЕСТО (клиника, стоматология, салон, барбершоп, студия, ресторан, кафе, отель, фитнес и т.п.). "
                      "Называй бизнес ЕГО словом из ниши (стоматология, клиника, салон, студия, ресторан, отель), а НЕ «компания». "
                      "Каждый запрос — про КОНКРЕТНУЮ услугу, процедуру или направление, а не про «услуги высокого уровня» или «полный спектр». "
                      "Перечисли реальные услуги ниши и спрашивай про них (для стоматологии: имплантация зубов, коронки, протезирование, брекеты, "
                      "чистка зубов, отбеливание, детская стоматология, КТ-снимок). "
                      "Живые формулировки: «Где сделать [процедуру] в [городе]?», «В какой клинике хорошо делают [процедуру]?», "
                      "«Какую [заведение] выбрать для [задачи]?», «Куда пойти за [услугой]?», «Хорошая [заведение] в [городе]».\n")
    else:
        role_block = ("\nЭТО УСЛУГИ ИЛИ ПОДРЯД. Называй бизнес: компания, агентство, студия, подрядчик. "
                      "Каждый запрос — про КОНКРЕТНУЮ услугу или направление (реальные услуги ниши), а не про «полный спектр услуг» и «комплексные решения».\n")
    ...
    prompt = (
        ...
        "НЕ ОБРАЩАЙСЯ К НЕЙРОСЕТИ И НЕ КОМАНДУЙ ЕЙ. Запрещены слова-обращения: «найди», «найдите», «посоветуй», «посоветуйте», "
        "«подскажи», «подскажите», «порекомендуй», «порекомендуйте», «помоги», «покажи», «перечисли», «назови», «дай список». "
        "Человек не разговаривает с нейросетью на «вы» и не просит её — он просто задаёт вопрос. Только безличные вопросы.\n"
...
_QSET_VER = "9"   # бамп при изменении промпта запросов -> старый кэш игнорируется и набор пересобирается
...
def _reco_examples(niche, brand_short, queries=None, site_info=None):
    ...
    prompt = (
        ...
        "сделан, для кого, чем выделяется, какие есть варианты. Конкретные факты (материал, размеры, варианты), неизвестное — в [скобках]. "
        ...
        "Состав: <3–4 пункта через «; »: что входит, длительность или формат, условия>\n"
        "PROJECT:\n"
        "<Готовое описание услуги или направления СПЛОШНЫМ текстом, 2–4 предложения, как реальный текст на сайте: что это, для кого, "
        "что входит, сколько длится, на каких условиях. Неизвестное — в [скобках]. БЕЗ полей «Поле: значение», без markdown — живой связный текст.>\n"
        "REVIEW:\n"
        "<готовый отзыв посетителя: что делали или заказывали, что понравилось, сервис; неизвестное — в [скобках]>"
...
def _recommendations(queries, groups, total, site_info=None, brand_short="бренд", niche=""):
    ...
    if product_biz:
        ...
        promo_fb = ("Полезный отзыв вместо «Всё понравилось»: "
                    f"«Заказывали [товар] в {b}: [материал и качество], доставка за [срок], всё подошло». "
                    "Чем конкретнее факты (что купили, как подошло, доставка), тем охотнее нейросеть процитирует отзыв.")
    elif place_biz:
        ...
        promo_fb = ("Полезный отзыв вместо «Всё понравилось»: "
                    f"«Были в {b}: [что именно делали или заказывали], понравилось [конкретика], сервис [оценка]». "
                    "Чем конкретнее факты, тем охотнее нейросеть процитирует отзыв.")
    else:
